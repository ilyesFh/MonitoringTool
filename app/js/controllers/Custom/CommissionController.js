...
app.controller('commissionController', function ($scope, $http, $filter , $interval ,$state) {
	...
	$scope.load = function () {
		...
		var msg = "{\"Var1\": " + "\"" + $scope.formattedDate + "\", \"Prefix\":\"Commission\"  }";
		console.log(msg);
		var res = $http.post('http://117.55.209.110:9080/ws/simple/getMoniPricingTool;boomi_auth=YXZheGlhLTlGQ0pJRjo3ZDA1NzAwZC1mODM1LTQ4NTUtOThjNC03OWFlMTc1OGRkYWI=', msg).
			then(function (response) {
				console.log(response.data[0]);
				$scope.getList = response.data[0];
...
app.controller('FSCommFileController', function ($scope, $http, $filter , $interval ,$state) {
	...
	$scope.exportData = function () {
		...
		if ( $scope.StatusSearch == undefined ) {
			...
			console.log("Normal filters");
			$scope.queryExport = 'SELECT Entry1 as Contract_Price_Code, Entry9 as Set_No, Entry5 as MoneyType , Entry7 as Prmomotion_Code  , Entry6 as PayeeCode , Entry10 as StartDate, Entry2 as EndDate , Entry8 as State , Entry3 as Filename  INTO XLSX("Report_GetDataProduction.xlsx",{}) FROM ? ' ;
		}
		else
		{
			console.log("advanced filters");
