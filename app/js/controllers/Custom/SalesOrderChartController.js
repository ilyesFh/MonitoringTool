    ...
    app.controller('DataTablesCtrl',  function($scope , $http ,  $filter , $timeout , $state , $modal, $log  ) {  
	   ...
	   $scope.showSourceHeader = false;
		...
		if( /*$scope.typeSearch == '' && */ $scope.StatusSearch == '' )
		{
			...
			$scope.queryExport = 'SELECT Entry14 as System_Source, Entry13 as So_Number, Entry12 as Plant, Entry4 as Delivery_Date , Entry6 as Sold_To , Entry9 as Creation_Date_System ,  Entry10 as Creation_Time_WorkTable, Entry2 as Boomi_Process_Time ,   case when Entry8 = \'0\' then \'Created In Bolton\' else ( case when Entry8 = \'1\' then \'Boomi In Progress\' else  ( case when Entry8 = \'2\' then \'IDoc Sent To SAP\' else ( case when Entry8 = \'3\' then \'Error Sending IDoc To SAP\' else  ( case when Entry8 = \'4\' then \'SO Posted in SAP\' else ( case when Entry8 = \'5\' then \'SO Failed in SAP\' else \'SO Received from SAP\' end) end) end)  end )  end ) end as Type  INTO XLSX("Report_SalesOrders.xlsx",{}) FROM ? ' ;
